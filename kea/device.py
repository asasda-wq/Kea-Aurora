...
class Device(object):
    ...
    def install_app(self, app):
        ...
        try:
            subprocess.check_call(["adb", "-s", self.serial, "uninstall", app.get_package_name()],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError:
            print(f"应用 {app.get_package_name()} 未安装，继续执行...")
        package_name = app.get_package_name()
        ...
        if self.output_dir is not None:
            ...
            package_info_file = open(package_info_file_name, "w",encoding="utf-8")
