...
class InputManager(object):
    ...
    def get_input_policy(self, device, app, master):
        if self.policy_name == POLICY_NONE:
            ...
            input_policy = LLMPolicy(device, app, kea=self.kea, number_of_events_that_restart_app = self.number_of_events_that_restart_app, allow_to_generate_utg = self.generate_utg, output_dir=self.output_dir)
        else:
            self.logger.warning(
