...
class UTG(object):
    ...
    def get_screenshots_by_structure(self, structure_str):
        if structure_str not in self.G2.nodes:
            return []

        states = self.G2.nodes[structure_str]['states']
        screenshot_paths = [state.screenshot_path for state in states]
        return screenshot_paths

    def export_structure_screenshot_gallery(self, output_dir=None):
        if output_dir is None:
            output_dir = os.path.join(self.device.output_dir, "structure_gallery")
        os.makedirs(output_dir, exist_ok=True)

        html_path = os.path.join(output_dir, "index.html")
        with open(html_path, "w", encoding="utf-8") as f:
            f.write("<html><head><title>Structure Screenshot Gallery</title>\n")
            f.write(
                "<style>body { font-family: Arial; } img { height: 200px; margin: 5px; border: 1px solid #ccc; } </style>\n")
            f.write("</head><body>\n")
            f.write(f"<h1>Structure Screenshot Gallery</h1>\n")
            f.write(f"<p>共 {len(self.G2.nodes)} 个结构</p>\n")

            # 按状态数量排序
            sorted_structures = sorted(
                self.G2.nodes(data=True),
                key=lambda item: len(item[1]['states']),
                reverse=True
            )

            for structure_str, data in sorted_structures:
                state_count = len(data["states"])
                f.write(f"<h2>Structure: {structure_str} （状态数: {state_count}）</h2>\n")
                for state in data["states"]:
                    rel_path = os.path.relpath(state.screenshot_path, output_dir)
                    f.write(f"<img src='{rel_path}' title='{state.state_str}' alt='{state.state_str}' />\n")
                f.write("<hr>\n")

            f.write("</body></html>")

        self.logger.info(f"结构截图页面已导出到：{html_path}")

    def get_G2_summary(self):
        num_structures = self.G2.number_of_nodes()
        num_structure_edges = self.G2.number_of_edges()

        # 每个 structure_str 下聚集的 state 数
        structure_state_count = {
            structure_str: len(data['states'])
            for structure_str, data in self.G2.nodes(data=True)
        }

        # 输出到文件
        if self.device and self.device.output_dir:
            output_path = os.path.join(self.device.output_dir, "state_diversity.txt")
            with open(output_path, "w", encoding="utf-8") as f:
                f.write(f"结构数: {num_structures}\n")
                f.write(f"结构间边数: {num_structure_edges}\n")
                f.write("每个结构包含的状态数:\n")
                for struct, count in sorted(structure_state_count.items(), key=lambda x: -x[1]):
                    f.write(f"{struct}: {count}\n")
            self.logger.info(f"结构统计信息已写入 {output_path}")