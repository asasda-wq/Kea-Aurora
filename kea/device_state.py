...
class DeviceState(object):
    ...
    def get_view_desc(self, view):
        content_description = self.__safe_dict_get(view, 'content_description', default='')
        view_reid = self.__safe_dict_get(view, 'resource_id')
        view_text = self.__safe_dict_get(view, 'text', default='')
        scrollable = self.__safe_dict_get(view, 'scrollable')
        view_desc = f'view'
        if scrollable:
        ...
        if content_description:
            view_desc += f' with description  "{content_description}"'
        if view_text:
            view_text = view_text.replace('\n', '  ')
            view_text = f'{view_text[:40]}...' if len(view_text) > 40 else view_text
            view_desc += f' with text "{view_text}"'
        if view_reid:
            view_reid = view_reid.replace('\n', '  ')
            view_reid = view_reid.split("/")[-1]
            view_desc += f' with id "{view_reid}"'
        return view_desc
    ...
    def get_described_actions(self):
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'clickable') and not (
                ...
                view_desc=f'- a clickable view'
                view_text=self.__safe_dict_get(self.views[view_id], 'text', default='')
                view_reid=self.__safe_dict_get(self.views[view_id],'resource_id')
                content_description = self.__safe_dict_get(self.views[view_id], 'content_description', default='')
                if content_description:
                    content_description = content_description.replace('\n', '  ')
                    content_description = f'{content_description[:40]}...' if len(
                        content_description) > 40 else content_description
                    view_desc += f' with description  "{content_description}"'
                if view_text:
                    view_text = view_text.replace('\n', '  ')
                    view_text = f'{view_text[:40]}...' if len(view_text) > 40 else view_text
                ...
                if view_reid:
                    ...
                    view_reid = view_reid.split("/")[-1]
                    view_desc+=f' with id "{view_reid}"'
                available_actions.append(TouchEvent(view=self.views[view_id]))
                view_desc += f'({len(view_descs)})'
                view_descs.append(view_desc)
                touch_exclude_view_ids.add(view_id)
                touch_exclude_view_ids = touch_exclude_view_ids.union(
                    self.get_all_children(self.views[view_id])
                ...
                if "org.y20k.transistor" in self.foreground_activity and \
                        self.views[view_id]['resource_id'] == "org.y20k.transistor:id/player_sheet":
                    available_actions.append(ScrollEvent(view=self.views[view_id], direction="UP"))
                    view_descs.append(f'- an action to scroll UP'+f'({len(view_descs)})')

                if "org.y20k.transistor" in self.foreground_activity and \
                        self.views[view_id]['resource_id'] == "org.y20k.transistor:id/station_card":
                    available_actions.append(ScrollEvent(view=self.views[view_id], direction="RIGHT"))
                    view_descs.append(f'- an action to scroll RIGHT'+f'({len(view_descs)})')
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'scrollable'):
                ...
                view_descs.append(f'- an action to scroll DOWN'+f'({len(view_descs)})')
                available_actions.append(
                    ScrollEvent(view=self.views[view_id], direction="LEFT")
                )
                view_descs.append(f'- an action to scroll LEFT'+f'({len(view_descs)})')
                available_actions.append(
                    ScrollEvent(view=self.views[view_id], direction="RIGHT")
                )
                view_descs.append(f'- an action to scroll RIGHT'+f'({len(view_descs)})')
        ...
        for view_id in enabled_view_ids:
            ...
            if self.__safe_dict_get(self.views[view_id], 'long_clickable') and not self.__safe_dict_get(
                ...
                view_desc = f'- a long_clickable view'
                view_text = self.__safe_dict_get(self.views[view_id], 'text', default='')
                view_reid = self.__safe_dict_get(self.views[view_id], 'resource_id')
                content_description = self.__safe_dict_get(self.views[view_id], 'content_description', default='')
                if content_description:
                ...
                if view_reid:
                    ...
                    view_reid = view_reid.split("/")[-1]
                    view_desc += f' with id "{view_reid}"'
                available_actions.append(LongTouchEvent(view=self.views[view_id]))
                view_desc += f'({len(view_descs)})'
                view_descs.append(view_desc)
        ...
        for view_id in enabled_view_ids:
            if self.__safe_dict_get(self.views[view_id], 'editable'):
                ...
                if random.random() < 0.5:
                    sample_text = st.text(alphabet=string.ascii_letters, min_size=0, max_size=8).example()
                view_desc = f'- a editable view '
                view_text = self.__safe_dict_get(self.views[view_id], 'text', default='')
                view_reid = self.__safe_dict_get(self.views[view_id], 'resource_id')
                content_description = self.__safe_dict_get(self.views[view_id], 'content_description', default='')
                if content_description:
                    content_description = content_description.replace('\n', '  ')
                    content_description = f'{content_description[:40]}...' \
                        if len(content_description) > 40 else content_description
                    view_desc += f' with description  "{content_description}"'
                if view_text:
                    view_text = view_text.replace('\n', '  ')
                    view_text = f'{view_text[:40]}...' if len(view_text) > 40 else view_text
                ...
                if self.__safe_dict_get(self.views[view_id],
                    ...
                    view_descs.append(f'- a view to search'+f'({len(view_descs)})')
                    available_actions.append(
                        SetTextAndSearchEvent(text=sample_text)
                    )
                    view_descs.append(f'- a view to set text and search'+f'({len(view_descs)})')
        ...
        for view_id in enabled_view_ids:
            ...
            if self.__safe_dict_get(
                ...
                view_desc=f'- a clickable or checkable view'
                view_text = self.__safe_dict_get(self.views[view_id], 'text', default='')
                view_reid = self.__safe_dict_get(self.views[view_id], 'resource_id')
                content_description = self.__safe_dict_get(self.views[view_id], 'content_description', default='')
                if content_description:
